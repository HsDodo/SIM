...
import (
	...
	"server/user/api/internal/handler"
...
func main() {
	cfYamlByte, err := utils.GetServiceConfigYamlByte("server", "user.api")
	if err != nil {
	...
	for _, endPoint := range c.Etcd.Hosts {
		err := etcd.DeliveryAddress(endPoint, cont.USER_API_ETCD_KEY, fmt.Sprintf("%s:%d", c.Host, c.Port))
		if err != nil {
